...
class CBHG(nn.Module):
    def __init__(self, K, in_channels, channels, proj_channels, num_highways):
        ...
        self.rnn = nn.GRU(channels, channels // 2,
...
class LSA(nn.Module):
    def __init__(self, attn_dim, kernel_size=31, filters=32):
        super().__init__()
        self.conv = nn.Conv1d(1, filters, padding=(
            kernel_size - 1) // 2, kernel_size=kernel_size, bias=True)
    ...
    def forward(self, encoder_seq_proj, query, t, chars):
        ...
        u = self.v(torch.tanh(processed_query +
                   encoder_seq_proj + processed_loc))
        u = u.squeeze(-1)
...
class Decoder(nn.Module):
    ...
    def forward(self, encoder_seq, encoder_seq_proj, prenet_in,
        ...
        rnn1_hidden_next, rnn1_cell = self.res_rnn1(
            x, (rnn1_hidden, rnn1_cell))
        if self.training:
        ...
        rnn2_hidden_next, rnn2_cell = self.res_rnn2(
            x, (rnn2_hidden, rnn2_cell))
        if self.training:
...
class Tacotron(nn.Module):
    def __init__(self, embed_dims, num_chars, encoder_dims, decoder_dims, n_mels,
        ...
        self.encoder_proj = nn.Linear(
            encoder_dims + speaker_embedding_size, decoder_dims, bias=False)
        ...
        self.register_buffer("stop_threshold", torch.tensor(
            stop_threshold, dtype=torch.float32))
    ...
    def forward(self, x, m, speaker_embedding):
        ...
        context_vec = torch.zeros(
            batch_size, self.encoder_dims + self.speaker_embedding_size, device=device)
