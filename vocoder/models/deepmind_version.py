...
class WaveRNN(nn.Module):
    ...
    def forward(self, prev_y, prev_hidden, current_coarse):
        ...
        hidden_coarse, hidden_fine = torch.split(
            hidden, self.split_size, dim=1)
    ...
    def generate(self, seq_len):
        with torch.no_grad():
            ...
            for i in range(seq_len):
                ...
                R_coarse_u, R_fine_u, \
                    R_coarse_r, R_fine_r, \
                    R_coarse_e, R_fine_e = torch.split(
                        R_hidden, self.split_size, dim=1)
                ...
                fine_input = torch.cat(
                    [prev_outputs, coarse_pred.unsqueeze(0)], dim=1)
                fine_input_proj = self.I_fine(fine_input)
                I_fine_u, I_fine_r, I_fine_e = \
                    torch.split(fine_input_proj, self.split_size, dim=1)
